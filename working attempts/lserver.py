...
def players_turn():
    ...
    if idnum == playerTurn and players_alive[idnum]:
        ...
        if idnum in eliminated:
            ...
            print("217 Number Players Alive =",numberPlayersAlive)
            send_to_all(tiles.MessagePlayerTurn(playerTurn))
            # old_eliminated.append(idnum)
            return
        ...
        if isinstance(msg, tiles.MessagePlaceTile):
            ...
            if board.set_tile(msg.x, msg.y, msg.tileid, msg.rotation, msg.idnum):
                ...
                tileid = tiles.get_random_tileid()
                connections[idnum].sendall(tiles.MessageAddTileToHand(tileid).pack())
...
def game_ready():
    ...
    return (len(live_idnums) > 1) #Means 3 players are connected
def game_over():
    global numberPlayersAlive
